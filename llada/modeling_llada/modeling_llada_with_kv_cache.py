        ...
        for block_idx, block in enumerate(self):
            ...
            if (
                ...
                x, cache = self._activation_checkpoint_fn(  # type: ignore
                    block, x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache, cache_kv=cache_kv
                )
            else:
                # shape: (batch_size, seq_len, d_model)
                x, cache = block(x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache, cache_kv=cache_kv)
    ...
    def forward(
        ...
        if self.config.block_group_size == 1:
            ...
            for group_idx, block_group in enumerate(self.transformer.block_groups):
                ...
                x, cache = block_group(
                    x, attention_bias=attention_bias, layers_past=layers_past, use_cache=use_cache, cache_kv=cache_kv
                )
                if attn_key_values is not None:
                    assert cache is not None
