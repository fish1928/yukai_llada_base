...
class RunModelCacheWithRefresh:
    ...
    def run_model_semi_cached_refresh_per_block(model, x, y, config_diffusion, *args, **kwargs):
        ...
        for id_block in range(num_blocks):
            ...
            idx_denoising = torch.arange(position_start, position_end, dtype=torch.long).to(x.device)
            quota_helper = BlockDiffusionQuotaHelper(mask_mask_blk, size_block)
            idx_refresh = refresher.get_refresh_idx(x, 0, id_block, return_sorted=True)
    ...
    def main2():
        calculator_kvsim = config.klass_save_kv_previous()
        calculator_kvsim.dump_all_in_one(
            config_aggregate.folder_output,
            config_aggregate.len_prompt,
            config_aggregate.len_target,
            config_aggregate.num_blocks,
            config_aggregate.type_fn,
            'block',
            config_aggregate.stamp
        )
    # end
# end class


class RunModelCached:
    @ torch.no_grad()
    ...
    @ torch.no_grad()
    def run_model_semi_cached_collect_seq(model, x, y, config_diffusion, *args, **kwargs):
        ...
        p_finalized = torch.zeros(x.shape, dtype=torch.float64, device=x.device)
        seq_finalized = torch.zeros(x.shape, dtype=torch.float64, device=x.device) - 1
        ...
        for id_block in range(num_blocks):
            ...
            for step in range(step_per_block):
                ...
                idx_sorted_by_conf = sorter.argsort(conf_snapshot, snapshot)
                num_unmask = quota_helper.get_quota(step)
                idx_transform = idx_sorted_by_conf[:, :num_unmask]  #idx_transform: (1,*), idx_denoising: (*)

                snapshot.materialize_by_idx_(idx_transform, conf_snapshot)
                snapshot.update_this(1, idx_src=idx_transform, idx_tgt=idx_denoising, y=x).update_this(1, idx_src=idx_transform, idx_tgt=idx_denoising, p_finalized=p_finalized)

                idx_transform_global = torch.gather(idx_denoising.unsqueeze(0), dim=-1, index=idx_transform)
                step_global = step + step_per_block * id_block
                seq_finalized.scatter_(1, idx_transform_global, step_global)
            # end for step
        # end for block

        return p_finalized[:, len_prompt:], seq_finalized[:, len_prompt:]
    # end


# end

class RunModelCacheSnapshot:
...
# end
    ...
    '''bug version need further investigation, using step file for block?'''
    @ torch.no_grad()
    def run_model_semi_cached_refresh_per_block(model, x, y, config_diffusion, *args, **kwargs):
        ...
        for id_block in range(num_blocks):
            ...
            idx_denoising = torch.arange(position_start, position_end, dtype=torch.long).to(x.device)
            quota_helper = BlockDiffusionQuotaHelper(mask_mask_blk, size_block)
            # idx_refresh = refresher.get_refresh_idx(x, 0, id_block, return_sorted=True, id_step_global=id_block)    # 5.09
            idx_refresh = refresher.get_refresh_idx(x, 0, id_block, return_sorted=True)
    ...
    filename = 'all_in_one_diff_32_64_4_abs_per_step_p_0326.json'
    with open(filename, 'r') as file:
        data_refresh = json.load(file)
    # end
    ...
    for id_batch, batch in enumerate(tqdm(loader)):
        ...
        conf = run_model_semi_cached_refresh_per_block(
            model,
            batch['ids_prompt_masked_full'].to(config.device),
            batch['ids_target_masked_full'].to(config.device),
            config,
...
class RunModelSemiCachedSnapshotQueryHAttn:
    def run_model_semi_cached_snapshot_query_h_attn(self, model, x, y, config_diffusion, *args, **kwargs):
