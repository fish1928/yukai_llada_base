...
from constants_llada import DTYPE_EVAL, NAME_MLP_FULLATTN
...
class RunModel:
        ...
        for id_block in range(num_blocks):
            ...
            for step in range(step_per_block):
                ...
                if step == 0 or step % step_refresh_remainder == 0:
                    ...
                    score_attn = score_attn[:, idx_in_attn, -idx_block.shape[-1]:].squeeze(1)   # (32,1,64) -> (32, 64)
                    score_attn = score_attn.transpose(0,1).unsqueeze(0) # (1,64,32)

                    mask_mask_current_no = ~(x[:,position_start:position_end] == id_mask).view(1, -1, 1).expand(1,-1, score_attn.shape[-1])    # (B, K)
                    score_attn.masked_fill_(mask_mask_current_no, torch.finfo(score_attn.dtype).min)

                    idx_denoising = (future_idx_selector.select_future_by_attn_full(score_attn) + position_start).squeeze(0)
                    idx_current = torch.cat([idx_refresh, idx_denoising])
    ...
    def run_one(self, model, tokenizer, config, *args, **kwargs):
        ...
        if self.mlp is None:
            loader_mlp = FutureIdxSelectorModelLoader(32, config.device)
            self.mlp = loader_mlp.load(NAME_MLP_FULLATTN).to(DTYPE_EVAL)
