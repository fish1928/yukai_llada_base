...
ID_TOKEN_MASK = 811 # '<|mask|>'
ID_TOKEN_PADDING = 30399 # '<|endoftext|>'
ID_TOKEN_EOT = 2 # '<|endoftext|>'
ID_TOKEN_BOT = 1 # '<|beginoftext|>'
ID_TOKEN_SEP = 4936
ID_TOKEN_UNK = 0
...
'''define diffullama & llada supportive functions'''

def get_anneal_attn_mask(seq_len, bsz, dtype, device, attn_mask_ratio=1.0):
...
def run_model(
    ...
    attention_mask = get_anneal_attn_mask(x.shape[1], x.shape[0], dtype=torch.bfloat16, device=model.device, attn_mask_ratio=1.0)
    ...
    for num_block in range(num_blocks):
        ...
        for step_per_block in range(steps_per_block):   # TODO: 1 -> steps_per_block
            ...
            # NOTE: dream need a one-positional shift for logits
...
if __name__ == '__main__':
    ...
    id_model_g = 'diffusionfamily/diffullama'
    id_mask_g = ID_TOKEN_MASK
    device_g = 'cuda:0'
    size_batch_g = 32
    folder_log_base = 'logs_ppl_conf_diffullama'
    ...
    config_g = AutoConfig.from_pretrained(id_model_g)
    config_g.use_cache = False
            ...
            for num_blocks_g in (8,4,1):
                for num_unmask_per_iter_g in (1,2,4):
                    ...
                    for batch in tqdm(loader):
                        ...
                        with open(f'{folder_log_base}/{len_prompt_g}-{len_target_g}-{num_blocks_g}-{num_unmask_per_iter_g}.diffullama', 'a+') as file:
